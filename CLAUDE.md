...
**As of 2026-08-19 (daily research pass — Glama ~73,773 (+683 vs. Aug 18 ~73,090); MCPToplist 100,958 cross-registry (Aug 10 snap; no new snapshot; 9 days stale); PulseMCP ~22,050+ (flat, Day 9 — "until mid-August" pause now 9 days past stated end date; step-jump still pending); AAIF Seoul blog recap STILL NOT PUBLISHED (Day 5 post-summit; Futurumgroup coverage only); MCPwned BH2026 slides: GitHub release live (Aug 7, 91 files/3 assets) but page rendering intermittent; no cataloged server named in any public coverage; DeepJack (Adversa AI + Proofpoint CursorJack — Cursor deeplink installs attacker-controlled MCP server after 1 click; no CVE; patched July 13; client-side; no catalog action); Xnurta MCP (Aug 13) + EDITED MCP (Aug 12) retail persona catalog candidates; SEP-2127 server card audit OVERDUE (WG closed Aug 14; `subregistry-audit` highest priority); security Day 54 clean):**
