...
- **Latest daily update:** [2026-08-12-mcp-ecosystem-update.md](./2026-08-12-mcp-ecosystem-update.md)
- **Last updated:** 2026-08-12 (daily research pass — **Glama ~70,857 (+641 vs Aug 11)**; MCPToplist 100,958 (Aug 10 snap; no new snapshot); PulseMCP ~22,070+ (flat); **HubSpot MCP August capability expansion: Leads record read + Conversations data + Help Desk ratings** (curate #1 ready); **GitHub Enterprise MCP Allowlists GA Aug 6** — central `allowedMcpServers`/`deniedMcpServers` in managed-settings.json, enforced for Copilot app + CLI + VS Code, fail-closed design; **GitHub MCP Server v1.9.0 released Aug 10** (semantic search, label ordering, PR improvements, find_duplicate tool); **Nutanix MCP Server for NCP announced Aug 10** — open-source, Prism v4 API, Claude Code + Copilot + Cursor, infra persona not catalogable; **Slack MCP + Skills Plugin for Claude Code/Cursor announced** + CLI 4.6.0; **Team Cymru Pure Signal MCP Server backfilled** — launched Apr 29, security-persona/subscription-gated, watch list; Docker: Aug governance blogs + Gordon custom-rules tab for MCP allow/deny; MCPwned slides still not published (Streamly Aug 14); AAIF Seoul summit tomorrow Aug 13–14; SEP-2127 WG closes tomorrow Aug 14 — server card audit trigger active; security Day 47 clean)
