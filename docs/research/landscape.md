...
- **Latest daily update:** [2026-08-16-mcp-ecosystem-update.md](./2026-08-16-mcp-ecosystem-update.md)
- **Last updated:** 2026-08-16 (daily research pass — **Glama ~72,328 (approx. flat; egress-blocked, using search index)**; MCPToplist 100,958 (Aug 10 snap; no new snapshot); **PulseMCP ~22,050+ (flat, Day 6 — "until mid-August" pause has reached its stated end date; rework ship imminent, expect step-jump)**; **AAIF Seoul blog recap STILL NOT PUBLISHED** (Day 2 post-summit; member announcement confirmed: 247 total orgs, 3G/33S/21A breakdown, first financial-services Gold members: Visa + Wells Fargo); **MCPwned slides still not public** (expected ~Aug 19–20; T-3 days; no cataloged vendor named); **Claude Code v2.1.233 (Aug 14)** ships MCP v2 serverless-host fix (infinite subscription stream reopening on timeout-enforcing hosts — affects serverless-deployed catalog vendors); **SEP-2127 server card audit OVERDUE** (WG closed Aug 14; `subregistry-audit` trigger active; GET `/.well-known/mcp.json` on all 19 servers pending); security Day 51 clean)
